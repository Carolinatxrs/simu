 ...
 */
	...
	$qn = $_GET['n'];		//recebe a ordem da questão (nq)
	$total = $_GET['t'];	//recebe o total de questões
	$qid = $_GET['qcod'];	//recebe o id da questão (qid)
	$cat_q = $_GET['c'];	//categoria da questão
	$exame_q = $_GET['ex'];	//tipo de exame
	$resp = $_POST['resp'];	//POST recebe o id da resposta do usuário (res)
	...
	if($resp == $opcoes_id_op){
		...
		$_SESSION['resposta'][$qn]  = "1"; //se a resposta for correta armazena 1
	}else{
		...
		/*array para detalhar resultado do usuário*/
		$_SESSION['resposta'][$qn]  = "X";	//se a resposta for errada armazena X
	}

	if($qn != $total){
		$qn++;	//incrementa a ordem e chama a proxima questao
		header("location:../view/personalizado_start.php?n=$qn&t=$total&idh=$id_his&c=$cat_q&ex=$exame_q");
	}else{
		...
		header("location:../view/resultado_personalizado.php?idh=$id_his");
	}
?>